...
% I. Load aperiodidc noise
cd([dpath '\sub-' sub '\ses-' ses])
sprintf(['\n \n' 'Processing Sub: ' num2str(sub) '...\n \n'])

% Load aperiodic data to calculate threshold
load aperiodic.mat            
...
for vin=1:Nvoxin   % only the 1925 voxels inside the cortex to save time
    ...
    for i=1:CC.NumObjects
        if length(CC.PixelIdxList{i})>timep_freq(end)*1       % 1--- only evaluate clust 
            temp = L==i;
...
save episocc_matrix episocc                % matrix of episode occurrences
...
subtime = toc;
disp(['Time for sub ' num2str(sub) ' was ' num2str(subtime/3600) ' hours'])
