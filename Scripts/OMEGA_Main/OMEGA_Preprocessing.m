function OMEGA_Preprocessing (sub, ses, rawpath, dpath)
...
%% 1.1. Reading data
    mkdir([dpath '\sub-' sub '\ses-' ses])
    cd([rawpath '\sub-' sub '/ses-' ses '/meg/'])
    
    datafile = findfile('resting');
    cd(datafile)
...
save([dpath '\sub-' sub '\ses' ses '\data_preproc.mat'], 'data');
...
save([dpath '\sub-' sub '\ses' ses '\comp.mat'], 'comp');
...
save([dpath '\sub-' sub '\ses' ses '\dataclean.mat'], 'dataclean');
