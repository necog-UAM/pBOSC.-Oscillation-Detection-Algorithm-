...
addpath('Z:\OMEGA\Enrique\Scripts\Base Scripts')
rawpath = 'Z:\OMEGA\OMEGA_raw\';
dpath = 'Z:\OMEGA\OMEGA_data'
addpath('Z:\OMEGA\Enrique')
addpath(genpath('Z:\Toolbox'))
locpath = 'C:\Users\Usuario\Desktop\WorkingDir\OMEGA_Repo\Scripts';
...
% correction: empty-room or aperiodic

% -------------------------------------------------------- %
% 5.1  Prepare and baseline correct signal and noise
...
for s = 2:Nsub;
    sub = subs{s};
    ses = sess{s};
    correction = 'aperiodic';

    % I. Load empty-room or aperiodidc noise
    cd([dpath '\sub-' sub '\ses-' ses])
    sprintf(['\n \n' 'Processing Sub:' num2str(s) '...\n \n'])

    if strcmp(correction, 'empty-room')
        load datasource_allvox_empty-room.mat
        emptynoise = dataclean.trial{1};
    elseif strcmp(correction, 'aperiodic')
        load aperiodic_uncorrected.mat
    end
    ...
    if strcmp(correction, 'aperiodic')
    ...
    elseif strcmp(correction, 'empty-room')

        % II. Cut empty-room to same length of original
        emptynoise = emptynoise(:,1:length(dataclean.time{1}));

        % III. Correction to center of the head bias
        dataclean.trial{1} = dataclean.trial{1} ./ rms(emptynoise,2);
        noise_corr = emptynoise ./ rms(emptynoise,2);
    ...
    % A. This thshld uses the 95% pctile of aperiodic
    thshld_noise = single(zeros(Nfrex,1));
    for f0 = 1:Nfrex
        thshld_noise(f0) = prctile(prctile(squeeze(powspctmnoise(:,f0,:)),95,1),95,2);
    end

    thshld_noise = repmat(thshld_noise, 1, size(powspctm,1));
    thshld_noise = permute(thshld_noise,[2 1]);

    tic
    % B. This thshld uses both distributions (signal and noise)
    thshld = Omega_thshld(powspctm, powspctmnoise, 0.95);
    step12 = toc;

    save thshld_noise thshld_noise
    save thshld thshld
    ...
    %Thshld1
    detpks3 = int8(zeros(Nvox,Nfrex,Ntp));
    % Thshld or thshld_noise
    for t=1:size(detpks,3)
        detpks2=detpks(:,:,t);
        powspctm2=powspctm(:,:,t);
        detpks2(powspctm2<thshld_noise) = 0;
        detpks3(:,:,t)=detpks2;
    end

    % Thshld2
    detpks4 = int8(zeros(Nvox,Nfrex,Ntp));
    % Thshld or thshld_noise
    for t=1:size(detpks,3)
        detpks2=detpks(:,:,t);
        powspctm2=powspctm(:,:,t);
    ...
    clear thshld_noise
    % Plot
    % figure, imagesc(dataclean.time{1},frex,squeeze(detpks(1357,:,:))), axis xy, title('Detpks 2 pow threshold')
    ...
    % %
    save epis_prctile95noise_1cyc epis
