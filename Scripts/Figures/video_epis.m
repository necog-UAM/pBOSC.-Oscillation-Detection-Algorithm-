...
load 'Z:\OMEGA\OMEGA_data\sub-0002\ses-0001\datasource_3423.mat'
load 'Z:\OMEGA\OMEGA_data\sub-0002\ses-0001\source_forward_10mm_3423.mat'
load 'Z:\OMEGA\OMEGA_data\sub-0002\ses-0001\source_inverse_10mm_3423.mat'

% Load aperiodic and correct
load 'Z:\OMEGA\OMEGA_data\sub-0002\ses-0001\aperiodic_orig.mat'
aperiodic = aperiodic_orig;
clear aperiodic_orig 
...
cd ('Z:\OMEGA\Enrique\Video')
Ncol = 12;
tt = sr*1;    % number of seconds per figure
ctv=1;
for t0 = 200:4:3000%length(time)
    tic
    t0
    ...
    for q = 1:Ncol
        if q==7   
            title([' Time: ' num2str(round(time(t0)+(time(tf)-time(t0))/2,2)) ' s '], 'FontSize', 18)
        end
        ...
        set(gca, 'YTick',flip(ytck),'YTickLabel',flip(ytcklab), 'Fontsize',5)
        ylim([y0 yf])
    end
    ...
    % colorbar
    subplot(4,Ncol,12)
    cbar = repmat(linspace(0, 1, 256)', 1, 20);  % Un vector de 256 valores repetidos en 20 columnas
    imagesc(cbar);
