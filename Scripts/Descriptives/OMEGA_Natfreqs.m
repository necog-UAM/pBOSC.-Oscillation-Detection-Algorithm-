...
addpath('Z:\OMEGA\OMEGA-NaturalFrequencies-main\mat_files')
...
load oscillatory_results_orig3cyc.mat
...
pBOSC_nii(nfmean, ['nf_mean_origAAA']);
pBOSC_nii(natfreqstd, 'nf_std_orig');
...
cd('Z:\OMEGA\Enrique\Results')
% load natfreq_bootstrap_orig
save natfreq_bootstrap_rand3cyc natf natfmu natfci natfstd
natfmu_btstrp = natfmu; % rename
natf_btstrp = natf;

% Figures
pBOSC_nii(natfmu_btstrp, 'natf_orig_bootstrap')

addpath('Z:\Toolbox\fieldtrip-20230118') % Path to Fieldtrip
colcfg = [];
colcfg.colim   = [0.7 3.4];  
colcfg.colmap   = 'jet_omega_mod';
pBOSC_sourcefig(log(natfmu_btstrp), 'NATF_ORIG3cyc',colcfg)
...
[rho,  pval] = corr(natfmu_btstrp, natfmu_Almu) % Entre los voxs: 0.5404. Con nb 0.6136, p=1.5299e-199
...
%% Natural frequencies ROIs with pBOSC
% Load roi str
...
% Rois with pbosc
pBOSC_nii(roinf, 'Roi_NatfreqKmeans_orig')
...
% Normalize data to [0, 1] for colormap indexing
dataNorm = (data - min(data(:))) / (max(data(:)) - min(data(:)));
[nRows, nCols] = size(data);

% Overlay text with adaptive color
for row = 1:nRows
    for col = 1:nCols
        % Get normalized value and corresponding RGB color
...
ax = gca;
ax.YTick = 1:size(data,1);                  % Keep Y tick positions
ax.YTickLabel = aal_label_reduc;            % Keep Y tick labels
ax.TickLength = [0 0];                      % Remove tick marks
ax.YAxis.TickDirection = 'out';             % Optional: move ticks outward (won't show if length is 0)
...
pBOSC_nii(diffnf, 'Diffs_ROIs_orig')

colcfg = [];
colcfg.colim   = [0 15];  
colcfg.colmap   = colormap('sky');
pBOSC_sourcefig(diffnf, 'test',colcfg)
